...
+ (void)getOrderAddressBook:(AddressBookDictBlock)addressBookInfo authorizationFailure:(AuthorizationFailure)failure
{
    ...
    dispatch_async(queue, ^{
        ...
        [kPPAddressBookHandle getAddressBookDataSource:^(PPPersonModel *model) {
            //获取到姓名的大写首字母
            NSString *firstLetterString = [self getFirstLetterFromString:model.name withSetAllPinYin:model];
...
+ (void)getAllAddressBook:(AllAddressBookArrayBlock)addressBookInfo authorizationFailure:(AuthorizationFailure)failure {
    // 将耗时操作放到子线程
    dispatch_queue_t queue = dispatch_queue_create("addressBook.infoDict", DISPATCH_QUEUE_SERIAL);
    ...
    dispatch_async(queue, ^{
        ...
        if ([nameKeys.firstObject isEqualToString:@"#"])
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                addressBookInfo ? addressBookInfo(array,addressBookDict,mutableNamekeys) : nil;
            });
            return;
        }
        ...
        // 将排序好的通讯录数据回调到主线程
        dispatch_async(dispatch_get_main_queue(), ^{
            addressBookInfo ? addressBookInfo(array,addressBookDict,nameKeys) : nil;
        });
...
+ (NSString *)getFirstLetterFromString:(NSString *)aString withSetAllPinYin : (PPPersonModel *) model
{
    /**
     * **************************************** START ***************************************
    ...
    // 去除拼音中的空格
    NSString *pyString = [pinyinString stringByReplacingOccurrencesOfString:@" " withString:@""];
    model.namePinYin = pyString;
